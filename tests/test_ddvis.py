...
def test_visualise_with_docker_success(mock_getcwd, mock_which, mock_exists, mock_makedirs, mock_chdir,
...
def test_visualise_with_docker_missing_docker_compose(mock_which):
    # Early return if docker-compose not available
    m.visualise_with_docker("schema.json")


@patch.object(m.shutil, "which", return_value="/usr/local/bin/docker-compose")
