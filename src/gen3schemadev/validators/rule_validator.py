...
class RuleValidator:
    ...
    _RULES = (
        "data_file_link_core_metadata",
        "link_props_exist",
        "props_must_have_type",
        "type_array_needs_items",
        "core_metadata_required_link",
        "data_file_props_need_data_props",
        "project_must_require_code",
    )

    def validate(self) -> list:
        """
        Run every rule and return the violations found, rather than stopping
        at the first one.

        Stopping at the first failure turned a dictionary with six problems
        into six edit-run-edit cycles, because each run only ever revealed the
        next problem. The rules are independent, so there is no reason the
        reader cannot see all of them at once.

        Returns:
            A list of {'schema', 'rule', 'message'} dicts, empty when the
            schema is clean.
        """
        ...
        for rule in self._RULES:
            try:
                ...
                # Two of the older rules re-wrap their own ValueError in a
                # RuntimeError. That wrapper adds a layer the reader has to
                # mentally strip, so report the original cause instead.
    ...
    def _get_links(self):
        """
        Return every concrete link in the schema, flattening nested subgroups.

        Gen3 allows a subgroup inside a subgroup - ``submitted_copy_number`` in
        the official Gen3 dictionary is shaped exactly that way. Unwrapping
        only the outer level left the inner group dict sitting in the list, and
        a group dict has no 'name', so link_props_exist reported a missing
        property for a link called 'None' and pointed the reader at a link that
        does not exist in their file.

        The previous version also returned only ``links[0]['subgroup']``, which
        silently discarded any further top-level entries - anything defined
        there was never checked at all.
        """
        def flatten(items):
            found = []
            for item in items or []:
                if not isinstance(item, dict):
                    continue
                if "subgroup" in item:
                    found.extend(flatten(item["subgroup"]))
                else:
                    found.append(item)
            return found

        links = flatten(self.schema.get("links", []))
    ...
    def link_props_exist(self):
        ...
        try:
            ...
            # Report every missing link at once. Naming one at a time meant a
            # node with three missing link properties took three runs to fix.
            missing = [name for name in link_name_list if name not in prop_keys]
            if missing:
                named = ", ".join(f"'{name}'" for name in missing)
                logger.error(
                    f"Properties for links {named} do not exist in properties of schema '{schema_id}'."
                )
                raise ValueError(
                    f"In schema '{schema_id}', property for link {named} "
                    f"is missing from the 'properties' section. "
                    f"Please add a property named {named} to resolve this."
                )
            logger.debug(
